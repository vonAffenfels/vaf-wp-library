...
final class AdminPagesModule extends AbstractHookModule
{
    ...
    final private function loadPages(): void
    {
        ...
        foreach ($this->laterChildMenus as $childMenu) {
            if (!$this->registerChildMenuItem($childMenu)) {
                throw new ParentMenuNotFound($this->getPlugin(), $childMenu->getMenu()->getParent());
            }
        }
    }
    ...
    final private function registerMenuItem(AdminPage $page): void
    {
        ...
        if (is_null($parent)) {
            ...
            if (!$this->registerChildMenuItem($page)) {
                // Menu item could not be added at this time
                // So we save it for it might be available later
                $this->laterChildMenus[] = $page;
            }
        }
    }
